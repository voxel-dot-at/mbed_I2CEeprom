...
size_t I2CEeprom::write(size_t address, const char *buffer, size_t size) {
    ...
    while (left != 0) {
        ...
        values[0] = address;
        memcpy(values+1, buffer+(size - left), toWrite);
        //printf("size - left:  %i\n", size - left);
        //printf("buffer[size - left]:  0x%02x\n", buffer[size - left]);
        printf("toWrite:  %i\n", toWrite);
        if (m_i2c.write(m_i2cAddress, values, toWrite+1) != 0) {
